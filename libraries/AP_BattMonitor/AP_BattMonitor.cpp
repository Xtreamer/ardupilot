...
AP_BattMonitor::read()
{
	...
	if (_aux_volt_enabled != AP_AUX_BATT_MONITOR_DISABLED) {		
		// this copes with changing the pin at runtime
		_aux_volt_pin_analog_source->set_pin(_aux_volt_pin);
		_voltage_aux = _aux_volt_pin_analog_source->voltage_average() * _aux_volt_multiplier;
	}    
}
...
bool AP_BattMonitor::exhausted_aux(float low_voltage, float min_capacity_mah)
{
	...
	// check voltage
	if ((_voltage_aux != 0) && (low_voltage > 0) && (_voltage_aux < low_voltage)) {
		// this is the first time our voltage has dropped below minimum so start timer
		if (_low_voltage_aux_start_ms == 0) {
			_low_voltage_aux_start_ms = tnow;
		} else if (tnow - _low_voltage_aux_start_ms > AP_BATT_LOW_VOLT_TIMEOUT_MS) {
			return true;
		}
	} else {
		// acceptable voltage so reset timer
		_low_voltage_aux_start_ms = 0;
	}
